...
double EncodeTool::FootpointLatitude(double y)
{
	...
	n = (sm_a - sm_b) / (sm_a + sm_b);
	alpha_ = ((sm_a + sm_b) / 2.0) * (1.0 + (pow(n, 2.0) / 4.0) + (pow(n, 4.0) / 64.0));
	y_ = y / alpha_;
...
void EncodeTool::MapLatLonToXY(double phi, double lambda, double lambda0, UTMCorr &xy)
{
	...
	N = pow(sm_a, 2.0) / (sm_b * sqrt(1.0 + nu2));
	t = tan(phi);
	t2 = t * t;
	...
	l5coef = 5.0 - 18.0 * t2 + (t2 * t2) + 14.0 * nu2 - 58.0 * t2 *t2;
...
void EncodeTool::MapXYToLatLon(double x, double y, double lambda0, WGS84Corr &philambda)
{
	...
	cf = cos(phif);
	nuf2 = ep2 * pow(cf, 2.0);
	Nf = pow(sm_a, 2.0) / (sm_b * sqrt(1.0 + nuf2));
	Nfpow = Nf;
	tf = tan(phif);
	tf2 = tf * tf;
	tf4 = tf2 * tf2;
...
void EncodeTool::LatLonToUTMXY(double lat, double lon, int zone, UTMCorr &xy)
{
...
void EncodeTool::UTMXYToLatLon(double x, double y, int zone, bool southhemi, WGS84Corr &latlon)
{
	double cmeridian;
	x -= 500000.0;
	x /= UTMScaleFactor;
